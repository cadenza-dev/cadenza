...
| rail collapse, side swap, drawer, pseudo-fullscreen state | `prototype-only` | `src/main.tsx`, layout/fullscreen guidelines | Interaction evidence only; not production layout persistence or browser API contract. |
| presenter-view mock state | `prototype-only` | `presenter-view-guideline.md`, `src/main.tsx` | Shows intended notes boundary; browser/desktop presenter guarantees remain Stage A research. |
