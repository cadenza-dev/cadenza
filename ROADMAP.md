...
- **Skills first, MCP later** — early AI investment goes into authorial voice through `.agents/skills/`; MCP is layered once it earns its place.
...
| **1. Core Semantics & Typed API** | Typed API primitives, state-to-timeline compiler, render-safe metadata, runtime intent, validation reports, first 5 skills | Frozen Phase 1 scenarios are green; the all-domain fixture compiles to deterministic preview/offline `TimelineMap` signatures; trace records the explicit boundary that real React/Remotion rendering, MP4 export, PDF export, external alpha usage, and public API stability are not claimed |
...
| **3. AI Authoring Strengthening** | Compile → error → repair loop, skill pack to 10+, thin IR if earned, optional read-only MCP | Agent-authored decks can be generated, previewed, diagnosed, and repaired through the local validation loop without hand-editing the core framework |
...
- **Phase 1 ships** when the semantic core is trace-complete: typed API, compiler, runtime intent, render-safe metadata, validation report, skills, and all-domain fixture.
