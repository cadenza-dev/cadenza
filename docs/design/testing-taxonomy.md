...
Cadenza intentionally uses two primary test homes:
...
| Repository artifact smoke | Prefer `scripts/check-*.ts`; existing small Vitest checks may stay package-local | `pnpm check:*` or `pnpm test` | Skill mirrors, harness shape, memory shape, frozen-contract policy, and other repo files. |
...
is not a pure package unit test. It remains package-local for now because the
current Vitest include is intentionally narrow:

```ts
include: ["packages/**/*.test.ts"]
```

If repository artifact smoke tests grow beyond a few checks, create
`tests/repo/*.test.ts`, update `vitest.config.ts`, and move repo-artifact
Vitest tests there as a deliberate migration.
...
- Repo-artifact Vitest tests grow enough to deserve `tests/repo/`.
